...
This file calculates all the data from the input file and outputs the information to the output file.
**/
...
Dna::Dna()
{
  ...
  sumBigrams = 0; /* takes the sum of values and sets it at zero so when you enter the correct DNA, it enables you to get the correct sum*/
  mean = 0; /* sets the mean at zero so then we can test for it as we enter the correct DNA strand*/
  variance = 0; /* variance set to zero so we can calculate the value out*/
  sd = 0;/*standard deviation set to zero to enable us, with the correct terms, to accurately acquire the standard deviation of the values*/
...
void Dna::readFile(string file)
{
  ...
  string line;
  cout << "wfuc" << endl;
  cout << "Trying to read file: " << file << ' ' << myFile.good() << '\n';
...
void Dna::outputData(string fileName)
{
  ...
  if (myFile.is_open())
  {
    myFile << "Christopher Galdi  2299616  CPSC350-01" << endl;
